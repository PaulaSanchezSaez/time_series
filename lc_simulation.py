...
def gen_DRW_long_DRM(seed,real_lag=100,dmag=0.5,time_range=2000,dtime=2,mag=19.5,errmag1=0.01,errmag2=0.03,tau=400,SFinf=0.2,sampling=False,timesamp1=0.0,timesamp2=0.0):
    ...
    y_obs1 = y+np.random.normal(0., np.random.normal(errmag1, 0.005), len(y))  #np.random.normal(y, errmag)

    ysig_obs2=np.ones(len(y))*errmag2
    y_obs2 = y+np.random.normal(0., np.random.normal(errmag2, 0.005), len(y))  #np.random.normal(y, errmag)
    ...
    if sampling:
        ...
        tstar = np.random.randint(14600,tbase-np.int((timesamp1[-1]-timesamp1[0])),size=1)
