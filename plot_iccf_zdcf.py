...
zdcf_path='../zdcf_stat/'
iccf_path='../iccf_stat/'
out_file_name='../iccf_stat/results_lowz_ws_type1_'+field+'_'+driving_filter+'_'+responding_filter+'_dt'+str(interp)+'.txt'
...
for i in xrange(len(ra)):
    ...
    if zspec[i]>0:
        ...
        ax3.text(0.15, 0.85, 'CCCD ICCF', horizontalalignment = 'center', verticalalignment = 'center', transform = ax3.transAxes, fontsize = 14)
        ax3.set_xlim(-400,900)
        print "iccf centdist plotted"

        plt.savefig('../cc_plots/results_zdcf_iccf_'+str(ras)+'_'+str(decs)+'_'+driving_filter+'_vs_'+responding_filter+'.png', format = 'png', orientation = 'landscape', bbox_inches = 'tight')
        plt.close('all')
